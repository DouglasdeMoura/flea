...
def test_single():
    ...
    single.click("Back")
    single.until("Back retained mark", lambda state: state["path"] == str(fixture) and len(state["marks"]) == 1)
...
def test_filters():
    ...
    filtered.key("-k", "End")
    filtered.until("last window loads", lambda state: state["cursorName"] == "zz-last.png")
...
def test_changed():
    ...
    changed.click("Open")
    changed.until("replacement removed with persistent error", lambda state: not state["marks"] and state["messageError"])
