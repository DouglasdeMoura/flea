...
# Usage: ./tests/picker.sh [pick|cancel|withdrawn|died|fault|taildrop]; taildrop is opt-in.
...
# The response code itself, which the caller collapses: 1 is the user's refusal and 2 is everything
# else, and omarchy-file-select turns both into exit 1. This is where the two can be told apart.
# Prints the code the portal answered with, or "none" when nothing answered at all.
response_code() {
    python3 - "$title" "$1" <<'ASK'
import sys
import gi
...
handle = bus.call_sync("org.freedesktop.portal.Desktop", "/org/freedesktop/portal/desktop",
                       ...
                       GLib.Variant("(ssa{sv})", ("", sys.argv[1], options)),
...
if sys.argv[2] == "close":
    GLib.timeout_add_seconds(6, withdraw)
...
case_withdrawn() {
    make_fixture
    response_code close > "$fixture/codes.txt"
...
case_died() {
    make_fixture
    response_code wait > "$fixture/died.txt" &
    local asker=$!
...
case_taildrop() {
    ...
    # panel's own cursor, and s is the panel's own send key. Neither is Flea's.
    omarchy-drive click-text "$peer" >/dev/null || fail "no row named $peer in the Tailscale panel"
...
backend_is_flea
[[ "$#" -gt 0 ]] || set -- pick cancel withdrawn died fault
for name in "$@"; do
    case "$name" in
        pick) case_pick ;;
        cancel) case_cancel ;;
