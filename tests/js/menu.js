...
function run(check) {
    ...
    check("fresh status explains why Taildrop cannot activate", entry(Menu.listingEntries(state({providersRefreshing: true})), "taildrop").hint, "Checking Tailscale")
    ...
    check("installed signed-out provider gives the actual reason", entry(Menu.listingEntries(state({taildropPeers: [], taildropReason: "Tailscale is signed out"})), "taildrop").hint, "Tailscale is signed out")
