...
function run(check) {
    ...
    check("offline Taildrop explains availability", offline.hint, "no peers")
    ...
    check("fresh status explains why Taildrop cannot activate", entry(Menu.listingEntries(state({providersRefreshing: true})), "taildrop").hint, "checking")
    ...
    check("installed signed-out provider gives the actual reason", entry(Menu.listingEntries(state({taildropPeers: [], taildropReason: "signed out"})), "taildrop").hint, "signed out")
