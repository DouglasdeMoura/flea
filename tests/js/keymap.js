...
function run(check) {
    ...
    // The five Finder chords the Mac preset overlays, so the preset is named here rather than
    // assumed: the map opens on Default, which claims no chord of its own.
...
// answers differently with the preset moved; Default and Vim overlay nothing, which is what that
// board's inventory says with a dash on the three view rows and a bare shared key on every other.
function runPreset(check) {
    var ctrl = Qt.ControlModifier
    var shift = Qt.ShiftModifier
    check("the preset this build opens on is Default", Keymap.preset, "default")
    check("Default claims no chord, so Finder's Ctrl+1 and Explorer's Ctrl+H are both unbound",
          [Keymap.lookup(Qt.Key_1, "1", ctrl), Keymap.lookup(Qt.Key_H, "\u0008", ctrl),
           Keymap.lookup(Qt.Key_Up, "", ctrl), Keymap.lookup(Qt.Key_K, "\u000b", ctrl)].join("|"),
          "|||")
    check("and the shared map is the whole of it, bare keys and chords alike",
    ...
    // Every vim key is shared and preset-independent, so Vim overlays nothing and answers as Default.
    Keymap.setPreset("vim")
    check("Vim answers exactly as Default does, because its rows are the shared ones",
          [Keymap.lookup(Qt.Key_1, "1", ctrl), Keymap.lookup(Qt.Key_H, "\u0008", ctrl),
           Keymap.lookup(Qt.Key_J, "j", Qt.NoModifier), Keymap.lookup(Qt.Key_G, "G", Qt.NoModifier)]
              .join("|"), "||cursorDown|cursorLast")
    ...
    check("a preset name this build does not have reaches no overlay row, which is Default's map",
          [Keymap.lookup(Qt.Key_1, "1", ctrl), Keymap.lookup(Qt.Key_H, "\u0008", ctrl),
