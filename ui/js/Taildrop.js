...
function status(raw, exitCode, error) {
    ...
    var data
    try { data = JSON.parse(raw) } catch (e) { return { peers: [], reason: "Tailscale returned invalid status." } }
    if (!data || typeof data !== "object") return { peers: [], reason: "Tailscale returned invalid status." }
    if (data.BackendState === "NeedsLogin") return { peers: [], reason: "Tailscale is signed out" }
    if (data.BackendState !== "Running") return { peers: [], reason: "Tailscale is " + String(data.BackendState || "unavailable") }
    ...
    if (!(self.CapMap && self.CapMap[capability] !== undefined)
        ...
        return { peers: [], reason: "Taildrop is disabled for this account" }
    var peers = parsePeers(raw)
    return { peers: peers, reason: peers.length ? "" : "No peers reachable" }
}
