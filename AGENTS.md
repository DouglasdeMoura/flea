...
text size, menus, the keybinding preset and places are the window's. **There is no second settings
file**: the settings panel's three sections write `display.textSize`, `menu` and `keys` in this same
document, through this same patch, so a value the panel shows and a value the backend reads cannot
be two different things. **The streams and the status are the contract**, because
...
- **`./tests/run-all.sh` is the one command, and it exists because nothing executed any suite
  ...
  the sixteen suites that need nothing but a shell, counted off the `headless` list the runner
  actually walks, and reads each suite's OWN exit code, never a pipeline's. It then names `ui.sh`,
  `drag.sh`, `bench.sh` and `package.sh` with what each
