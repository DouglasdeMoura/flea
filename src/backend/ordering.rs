...
pub fn request(l: &mut Listing, base: &Path, mime: &Db, line: &str) -> Result<(f64, f64), &'static str> {
...
pub fn ordered(l: &mut Listing, base: &Path, mime: &Db, by: &str, desc: bool, folders: bool, groups: bool) -> Result<(f64, f64), &'static str> {
    ...
    let kinds: Vec<&str> = if by == "kind" || groups {
        (0..l.len()).map(|i| if l.is_dir(i) { "inode/directory" } else { mime.lookup(l.name(i)).unwrap_or("application/octet-stream") }).collect()
    } else { Vec::new() };
    let mut indices: Vec<usize> = (0..l.len()).collect();
    indices.sort_by(|&a, &b| {
        let group = if groups {
            group_rank(l.is_dir(a), kinds[a]).cmp(&group_rank(l.is_dir(b), kinds[b]))
        } else if folders { l.is_dir(b).cmp(&l.is_dir(a)) } else { Ordering::Equal };
        ...
        let key = match by {
            ...
            "mtime" => { let stats = stats.as_ref().unwrap(); stats[a].mtime.cmp(&stats[b].mtime) }
            _ => Ordering::Equal,
        };
...
fn group_rank(directory: bool, mime: &str) -> u8 {
    if directory { 0 } else if mime.starts_with("image/") { 1 } else { 2 }
}
...
mod tests {
    ...
    fn folder_toggle_and_grouping_change_real_order() {
        ...
        d.dir("z-folder"); d.file("b.txt", "text"); d.file("c.jpg", "photo");
        let mut l = Listing::new();
        l.push("z-folder", true); l.push("b.txt", false); l.push("c.jpg", false);
        ...
        assert_eq!(l.name(0), "z-folder"); assert_eq!(l.name(1), "c.jpg");
