// The wire's requests: the one enum every command line parses into, and the parser that builds it.
// The responses are one format! line each and live in proto.rs and rows.rs; this file came out of
// proto.rs when the delete request pushed it past the hard cap, because the request side and the
// response side are two jobs over the same wire and only one of them was growing.
