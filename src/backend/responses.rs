// The wire's one-line responses: every answer that is a single format! line. Moved out of
// proto.rs whole when the trash browser's requests put it over the hard cap; the seam is the
// sentence the file already carried, that every other response is one format! line. proto.rs keeps
// the Request type and the request dispatch, so parsing lives on one side and answering on the other.
