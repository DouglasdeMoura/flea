...
impl Completion {
    pub fn new(settings: &Json) -> Self {
        ...
        Self { places: places(settings, &home), home, pending: None, running: None }
    }
    pub fn request(&mut self, value: &str, current: &Path) -> String {
        if let Some((cancel, _)) = &self.running { cancel.store(true, Ordering::Relaxed); }
        self.pending = None;
        if value.is_empty() { return String::new(); }
        if value == "~" { return "~/".into(); }
        if value == "." || value == ".." { return format!("{}/", value); }
        if let Some(path) = place(value, &self.places) { return path; }
    ...
    fn start(&mut self) {
        ...
        let Some((value, current)) = self.pending.take() else { return };
        let home = self.home.clone();
        let cancel = Arc::new(AtomicBool::new(false));
        let stopped = cancel.clone();
        let (tx, result) = mpsc::channel();
        std::thread::spawn(move || { let _ = tx.send(directory(&value, &current, &home, &stopped)); });
...
fn directory(value: &str, current: &Path, home: &Path, cancel: &AtomicBool) -> String {
    if cancel.load(Ordering::Relaxed) { return String::new(); }
    ...
    for entry in entries {
        ...
        if name.starts_with(prefix) && first.as_ref().map_or(true, |held| name < *held) && entry.file_type().is_ok_and(|t| t.is_dir()) { first = Some(name); }
    }
    let Some(name) = first else { return String::new() };
...
mod tests {
    ...
    fn completion_follows_prefix_without_importing_or_rewriting_places() {
        ...
        assert_eq!(directory("Do", root.path(), root.path(), &cancel), "Documents/");
        assert_eq!(directory("Dow", root.path(), root.path(), &cancel), "Downloads/");
        assert_eq!(directory("missing/", root.path(), root.path(), &cancel), "");
        assert_eq!(expand("~/Documents", root.path(), Path::new("/")), root.join("Documents"));
        assert_eq!(place("w", &[("Work".into(), "/data/work".into())]), Some("/data/work".into()));
    ...
    fn a_replaced_completion_cannot_land_on_the_new_query() {
        ...
        let mut completion = Completion { places: vec![("Home".into(), "/home/test".into())], home: "/home/test".into(), pending: None, running: Some((cancel.clone(), receiver)) };
        assert_eq!(completion.request("Home", Path::new("/fixture")), "/home/test");
