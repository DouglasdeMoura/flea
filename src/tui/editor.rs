...
impl Editor {
    pub fn new(kind: &'static str, value: String, path: PathBuf) -> Self {
        ...
        Self { kind, value, path, cursor, anchor: cursor, pending: false, error: String::new(), identity: None }
    }
    pub fn rename(value: String, path: PathBuf, directory: bool) -> std::io::Result<Self> {
        ...
        editor.cursor = if directory { editor.value.len() } else {
            editor.value.rfind('.').filter(|at| *at > 0).unwrap_or(editor.value.len())
        };
        editor.anchor = 0;
        Ok(editor)
    }
    pub fn valid(&mut self) -> bool {
        if matches!(self.kind, "rename" | "mkdir" | "newfile") && !crate::backend::ops::valid_name(&self.value) {
            self.error = "Enter one filename without a path separator.".into();
            return false;
        }
        if self.kind == "rename" {
            match self.path.symlink_metadata() {
                Ok(meta) if self.identity == Some((meta.dev(), meta.ino(), meta.mode() & 0o170000)) => {}
                _ => { self.error = "Selected item changed; reopen Rename.".into(); return false; }
            }
        }
        true
    }
    ...
    pub fn update(&mut self, key: &Key) {
        ...
        if command && key.name == "A" { self.anchor = 0; self.cursor = self.value.len(); return; }
        let start = self.cursor.min(self.anchor);
        let end = self.cursor.max(self.anchor);
        let previous = self.value[..self.cursor].char_indices().next_back().map_or(0, |(i, _)| i);
        let next = self.cursor + self.value[self.cursor..].chars().next().map_or(0, char::len_utf8);
        match key.name.as_str() {
            "Left" | "Right" | "Home" | "End" => {
                self.cursor = match key.name.as_str() {
                    "Home" => 0, "End" => self.value.len(),
                    "Left" => if !shift && start != end { start } else { previous },
                    _ => if !shift && start != end { end } else { next },
                };
                if !shift { self.anchor = self.cursor; }
            }
            "Backspace" | "Delete" if key.mods.is_empty() => {
                let range = if start != end { start..end } else if key.name == "Backspace" { previous..self.cursor } else { self.cursor..next };
    ...
    pub fn line(&self, prefix: &str, suffix: &str, columns: usize, base: &str, muted: &str) -> String {
        ...
        let mut first = 0;
        while first < self.cursor && render::text_width(&self.value[first..self.cursor]) >= available.max(1) {
        ...
        for (offset, c) in self.value[first..].char_indices() {
            ...
            if offset == self.cursor && start == end && used < available { out.push('▏'); used += 1; }
            let cells = render::text_width(&c.to_string());
            if used + cells > available { break; }
            if offset >= start && offset < end { out.push_str("\x1b[7m"); }
            out.push(c);
            if offset >= start && offset < end { out.push_str(base); }
            used += cells;
        }
        if self.cursor == self.value.len() && start == end && used < available { out.push('▏'); used += 1; }
        out.push_str(muted);
        let tail = if self.error.is_empty() { suffix.to_owned() } else { format!(" · {}", self.error) };
