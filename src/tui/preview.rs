...
pub fn load(m: &mut Model, force: bool) {
    ...
    if !row.kind.to_lowercase().contains("text")
        && !row.kind.to_lowercase().contains("document")
        && !row.kind.to_lowercase().contains("source")
    {
        return;
    }
    let before = match std::fs::symlink_metadata(&path) {
        Ok(v) => v,
        Err(_) => {
            m.preview.push("Could not read preview".into());
            return;
        }
    };
    ...
    let capped = body.len() as u64 > TEXT_BYTES;
    body.truncate(TEXT_BYTES as usize);
    if body.contains(&0) {
        return;
    }
    ...
    m.preview
        .extend(text.lines().take(TEXT_LINES).map(super::render::clean));
    if capped || text.lines().count() > TEXT_LINES {
        m.preview
            .push("Preview truncated · 256 KB or 2,000 lines".into());
    }
}
#[cfg(test)]
mod tests {
